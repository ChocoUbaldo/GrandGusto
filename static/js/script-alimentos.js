...
const bebidasRef = collection(db, "alimentos");
...
form_action.addEventListener('submit', async function(event) {
    ...
    // Realiza una consulta para verificar si un documento con el mismo nombre ya existe
...
// Verifica si se encontró un producto con el mismo nombre antes de agregar el nuevo producto a la base de datos
if (!productoExistente) {
    // Sube la imagen a Firebase Storage y agrega los datos a Firestore
...
table_body.addEventListener('click', async e => {
    if (e.target.classList.contains('edit')) {
        ...
        const docRef = doc(db, 'alimentos', productId);
